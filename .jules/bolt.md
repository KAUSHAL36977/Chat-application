...
## 2026-06-05 - Mongoose Array Document Retrieval Overhead
**Learning:** Using `findOne()` with `$or` conditions to validate fields (e.g., username or email) returns the full document for validation, adding unnecessary projection payload and memory overhead.
**Action:** Use concurrent `Promise.all([Model.exists(...), Model.exists(...)])` queries when only checking for existence of distinct fields to return specific validation errors, avoiding document hydration and unnecessary memory use.
