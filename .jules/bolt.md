...
## 2026-06-03 - Instance Method Mongoose Updates
**Learning:** Using `await this.save()` inside a Mongoose schema instance method triggers full document validation and `pre('save')` hooks which add unnecessary overhead when only updating a single non-critical field like a timestamp.
**Action:** Use `await this.model('ModelName').updateOne({ _id: this._id }, { $set: { field: value } })` within instance methods to optimize single-field writes, avoiding the validation/hydration pipeline overhead.
